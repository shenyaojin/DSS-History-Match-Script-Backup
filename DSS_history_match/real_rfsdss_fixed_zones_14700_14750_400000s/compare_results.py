...
lines=['# RFSDSS 固定区间反演对比','',
       '14700–14750 ft；0–400000 s；5 个区间独立反演渗透率，位置及背景固定。两组初值和物理模型相同。','',
...
lines+=['','本轮采用旧实测模板默认的 80 ft 井间距，未经测井轨迹验证；直接用 με×1e-6，无额外校准倍率。0–400000 s 包含主要压力上升及后期下降，结果仍以当前井间距和校准假设为条件。',
...
tmp=HERE/f'COMPARE_rectification.md.{os.getpid()}.tmp'
tmp.write_text('\n'.join(lines)+'\n');tmp.replace(HERE/'COMPARE_rectification.md')
print('Comparison written.')

subprocess.run([sys.executable,str(HERE/'make_deliverables.py')],check=True)
