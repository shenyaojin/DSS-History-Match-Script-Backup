...
def plot_qc(run, strain_csv, theta, label):
    ...
    for ax,values,title in zip(axs[:3],[obs,sim,sim-obs],['Observation','Simulation (same transform)','Residual: simulation − observation']):
        ...
        ax.set(title=title,xlabel='Time [s]',ylim=(14750,14700),xlim=(0,400000))
...
class Inversion:
    ...
    def optimize(self):
        self.validate()
        accepted=0
        def record_iteration(theta,index):
            value,_=self.cache[tuple(theta)]
            alpha=self.expand(theta)
            regularization=self.cfg['beta_l1']*np.sum(np.sqrt((alpha+18.)**2+.05**2)-.05)
            record=dict(iteration=index,scaled_objective=value,
                        data_objective=value/self.cfg['objective_scale']-regularization,
                        regularization=regularization)
            record.update({f'theta_{i+1}':float(v) for i,v in enumerate(theta)})
            pd.DataFrame([record]).to_csv(self.run/'accepted_iteration_history.csv',index=False,
                                         mode='w' if index==0 else 'a',header=index==0)
        record_iteration(self.theta0,0)
        def checkpoint(theta):
            nonlocal accepted
            accepted+=1
            np.savetxt(self.run/'checkpoint_theta.txt',theta)
            record_iteration(theta,accepted)
