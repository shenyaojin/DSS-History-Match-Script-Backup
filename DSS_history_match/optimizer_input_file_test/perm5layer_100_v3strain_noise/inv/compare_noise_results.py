...
LEVELS = [
    ("clean (v2)", os.path.join(HERE, "..", "..", "perm5layer_100_v2strain", "inv")),
    ("0.5%", os.path.join(HERE, "noise_0p5pct")),
    ("1%", os.path.join(HERE, "noise_1pct")),
    ("2%", os.path.join(HERE, "noise_2pct")),
    ("5%", os.path.join(HERE, "noise_5pct")),
]
...
def summarize(label, which, alpha, truth):
...
def main():
    ...
    for label, run_dir in LEVELS:
        ...
        for which, fname in variants:
            ...
            if alpha.shape[0] != TOTAL_LAYERS:
                ...
                continue
            rows.append(summarize(label, which, alpha, truth))
            if which == "final":
                final_profiles[label] = alpha
    ...
    for color, (label, alpha) in zip(cmap, final_profiles.items()):
        ax.plot(alpha, y_center, lw=1.6, color=color, label=f"L1 final, {label}")
