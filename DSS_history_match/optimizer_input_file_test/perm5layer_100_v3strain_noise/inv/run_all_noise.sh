#!/usr/bin/env bash
# Run the L1 inversion for each staged noise run folder, sequentially.
#
# Covers both families:
#   inv/noise_<tag>/      (peak-referenced noise: 0.5/1/2/5%)
#   inv/mednoise_<tag>/   (median-referenced noise: 1/2/5/10%)
#
...
#
# Usage:
#   bash run_all_noise.sh                       # run every staged folder
#   bash run_all_noise.sh noise_1pct mednoise_5pct   # run only the named folders
#   bash run_all_noise.sh mednoise_*            # shell-glob a subset
...
if [[ ${#FOLDERS[@]} -eq 0 ]]; then
  echo "ERROR: no staged run folders found. Run setup_noise_runs.sh first." >&2
  exit 1
fi
