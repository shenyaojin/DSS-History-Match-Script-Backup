...
└── 03_forward_qc_strain/             ← 每个 run 的"观测 vs 模拟应变"证据图
    ...
    ├── v4_rfsdss_0p5pct__qc_strain.png    (rfsdss 族 5 个)
    ├── v4_rfsdss_1pct__qc_strain.png
    ├── v4_rfsdss_2pct__qc_strain.png
    ├── v4_rfsdss_5pct__qc_strain.png
    └── v4_rfsdss_10pct__qc_strain.png
```

> **命名约定**：所有文件都加了 `v3_` / `v4_` 前缀标明出处；`__qc_strain` 表示"观测/模拟/残差/alpha 四联 QC 图"，`__alpha_profiles` 表示反演过程中的 alpha 剖面演化图。**v3 与 v4 的前向 QC 图现已全部齐备**（v4 五档反演已全部跑完并做完前向 QC）。
...
- **v4**：clean + rfsdss 全 5 档，全实线（**已含全部完成的反演**）。从图上可直接读出退化：0.5%/1%（蓝/青）贴合真值黑线；5%/10%（黄绿）在裂缝区**过冲**（alpha 冲到 −14 甚至 −13）、并在本应为 −18 的基质带（y∈[−15,12]）冒出**虚假活跃尖峰**——L1 正则已压不住高噪声引入的假异常。
...
- `v4_rfsdss_{0p5,1,2,5,10}pct__qc_strain.png`：**rfsdss 族 5 个（已补齐）**。低档残差淡、alpha 剖面贴合真值；高档残差与 alpha 剖面明显偏离，直观印证下表指标。
...
全 5 档反演已完成。下表 `final`（末次迭代解）与 `best_data`（数据失配最小解）并列，突出高噪时二者分叉：
...
| rfsdss 2% | final | −16.350 | 3.480 | 0.04434 | 0.06381 |
| rfsdss 2% | best_data | −14.525 | 0.475 | 0.00410 | 0.00590 |
| rfsdss 5% | final | −15.618 | 3.491 | 0.04077 | 0.05868 |
| rfsdss 5% | best_data | −15.492 | 3.595 | 0.03915 | 0.05633 |
| rfsdss 10% | final | −16.376 | 3.497 | 0.04969 | 0.07151 |
| rfsdss 10% | best_data | −16.198 | 3.821 | 0.04836 | 0.06959 |

**读表结论：**
- **0.5% 档反演几乎无损**（rel_l2 与 clean 基线相当，甚至略优）。
- **1% 起 final 与 best_data 明显分叉**——说明高噪时"最后一步"开始过拟合噪声，best_data（数据失配最小的更早迭代点）更可靠。最戏剧的是 **rfsdss 2%**：`best_data` 的 `rel_l2_free=0.0059`（几乎无损）但 `final` 已崩到 `0.0638`——同一档，选点不同、结果天差地别。
- **2% / 5% / 10% 的 final 全部崩坏**：`max_alpha_err` 都在 3.5 左右（某些层 alpha 偏差达 3.5 个数量级），`fracture_mean` 从 −14.5 掉到 −15.6 ~ −16.4。注意 final 指标在 2%/5%/10% 之间**不再单调**（5% 的 rel_l2 甚至略低于 2%）——因为这是含尖峰的噪声实现 + 优化在不同点停机，高噪区已进入"看运气"的失控区。
- 这印证了 v4 的价值：**结构化的真实噪声（尖峰+漂移）在高档位对反演的破坏，比 v3 同幅值均匀白噪声更严重、也更不稳定**；实用上应优先采信 `best_data` 而非 `final`。

---

## 6. 运行状态

> **v4 五档反演已全部完成，前向 QC 图已全部生成，本图集为完整终稿。**

| run | 状态 | 迭代数 | qc_strain 图 |
|---|---|---|---|
| rfsdss_0p5pct | ✅ 完成 | 34 | ✅ |
| rfsdss_1pct | ✅ 完成 | 45 | ✅ |
| rfsdss_2pct | ✅ 完成 | 142 | ✅ |
...
`02_inversion_comparison/v4_alpha_overlay.png` 与 `v4_comparison_summary.csv` 已用全部 5 档结果刷新；`03_forward_qc_strain/v4_rfsdss_*__qc_strain.png` 五张已补齐。

> 备注：这套反演在高噪档收敛很慢（2%/5% 各跑了 3 天量级、130+ 迭代），glob 遍历顺序是 `0p5→10→1→2→5`（字母序），若日后重跑想让最难的垫底、可把 `run_all_rfsdss.sh` 的默认顺序改成按噪声从小到大。
