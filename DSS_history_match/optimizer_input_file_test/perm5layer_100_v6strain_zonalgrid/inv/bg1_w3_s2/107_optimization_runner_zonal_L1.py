...
def strip_exodus(content):
    """Remove the [exodus] output sub-block from a forward input.

    The inversion never reads the Exodus field dump; writing it costs ~3 GB and
    significant wall time PER MOOSE solve. The [csv] block (which carries the
    objective + per-layer gradient reporters we DO read) is left intact.
    """
    ...
    if n == 0:
        print("WARNING: [exodus] block not found; forward will still write Exodus.")
    return new_content
...
def objective_and_gradient(theta):
    ...
    with open(QUIET_FORWARD_FILE, "w") as f:
        f.write(strip_exodus(quiet_moose_content(fwd_content)))
