...
(strain_yy objective, dipole adjoint source, mobility-corrected gradient VPP).
...
def run_moose_and_get_objective_and_gradient(alpha_vector, tag=""):
    ...
    # optimize.i is written assuming the runner stages into inv/inv_output/, so
    # '../foo' resolves to inv/foo. Here we run directly from inv/, so rewrite
    # those references to absolute paths.
...
if __name__ == "__main__":
    ...
    print("SUMMARY")
    print("=" * 60)
    print(f"{'Layer':>6}  {'y':>7}  {'FD grad':>14}  {'Adjoint grad':>14}  {'Adj/FD':>12}  {'Rel err':>10}")
    print("-" * 80)
    for r in results:
        print(f"{r['layer']:>6}  {r['y_center']:>7.2f}  {r['fd_grad']:>14.6e}  "
              f"{r['adjoint_grad']:>14.6e}  {r['ratio']:>12.4e}  {r['rel_error']:>9.4f}")
    ...
    print("  If 'Adj/FD' is consistent across layers (e.g. ~1e-6), there is a")
    print("  uniform missing prefactor (likely the rho/mu mobility); the new")
    print("  PorousFlowOptimizationAnisotropicDiffusionInnerProduct VPP fixes this.")
