...
for name in names:
    ...
    # Preserve the erroneous cached selection for traceability before repair.
    if not (p/'strain_final_wrong_t0.csv').exists():shutil.copy2(old,p/'strain_final_wrong_t0.csv')
...
for row,(r,label) in enumerate(zip(results,labels)):
    ...
    ax.set(title=f'{label} | Observed DSS',xlabel='Time [s]',ylabel='Measured depth [ft]',xlim=(0,40000),ylim=(14750,14700))
...
for r,label,color in zip(results,labels,['#2864a4','#b85c1e']):
    h=r['h'];track=h[h.purpose.isin(['initial','optimize'])].copy().reset_index(drop=True)
    track['evaluation_index']=np.arange(len(track));track['relative_misfit']=track.data_objective/track.data_objective.iloc[0]
...
for ax in axs:
    ax.set_xlabel('Objective evaluation index (0 = initial; includes line searches)');ax.grid(alpha=.25);ax.legend()
...
fig.suptitle('Misfit history | gradient-check probes and duplicate final solve excluded',fontsize=14)
