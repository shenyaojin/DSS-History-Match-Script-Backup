...
with np.load(SOURCE/'unrectified.npz',allow_pickle=True) as f:
    md=f['daxis'];times=f['taxis'];start=f['start_time'].item(); obs=f['data']*1e-6
...
for name in ['unrectified','half_wave_positive']:
    ...
    cfg=dict(zones=zones,zone_layer_indices=masks,initial_theta=[z['alpha'] for z in zones],
             ...
             start_time=str(start),end_time=float(times[-1]),requested_end_time=40000.,
