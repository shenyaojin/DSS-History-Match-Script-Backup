#%% I'll try to perform history match with a more delicate time curve and other small tweaks.
# Shenyao Jin, shenyaojin@mines.edu
# Note:
# Revised version of 102 (Not 104)
# To test the bad mesh size will impact the convergence OR NOT.
# ONLY fix the grid size, from ft to meter.
# NOT WORKING. Convergence failure before stp 52. Might need to implement JFNK Solver?
...
draft_val = np.median(DSSdata.data[ind,:],axis=0)
DSSdata.data -= draft_val.reshape((1, -1))
...
output_dir = "output/0929_history_match"
os.makedirs(output_dir, exist_ok=True)

input_file_name = os.path.join(output_dir, "DSS_history_match.i")

builder = ModelBuilder(project_name="DSS_history_match")
...
# Tweak the domain size to make sure the pressure wave will not reach the boundary
domain_bounds = (-600 * conversion_factor, 900 * conversion_factor)
...
builder.build_stitched_mesh_for_fractures(
    ...
    nx = 150,
    ny_per_layer_half= 70,
    bias_y=1.3
)

#%% --- 3. Define Materials and Geometry ---
matrix_mats = ZoneMaterialProperties(porosity=0.01, permeability="'1E-22 0 0  0 1E-22 0  0 0 1E-22'")
srv_mats = ZoneMaterialProperties(porosity=0.1, permeability="'1E-17 0 0  0 1E-17 0  0 0 1E-17'")
fracture_mats = ZoneMaterialProperties(porosity=0.1, permeability="'1E-15 0 0  0 1E-15 0  0 0 1E-15'")
...
gauge_data_for_moose.down_sample(2) # Downsample the gauge data for better I/O performance
...
viz_utils.plot_point_samplers(folder=output_dir, output_dir="figs/09292025")
viz_utils.plot_vector_samplers(folder=output_dir, output_dir="figs/09292025")
