...
# After I diagnose where the bug is, this file should be read-only. --Shenyao
# First run: 10042025, 11:37 PM, Savefile: output/1004_constDT_bug_investigation_successful.
#   - Successful run. No convergence failure. --Shenyao
#
...
matrix_mats = ZoneMaterialProperties(porosity=0.01, permeability="'1E-19 0 0  0 1E-19 0  0 0 1E-19'")
srv_mats = ZoneMaterialProperties(porosity=0.1, permeability="'1E-17 0 0  0 1E-17 0  0 0 1E-17'")
fracture_mats = ZoneMaterialProperties(porosity=0.1, permeability="'1E-14 0 0  0 1E-14 0  0 0 1E-14'")

builder.set_matrix_config(MatrixConfig(name="matrix", materials=matrix_mats))

center_x_val = domain_length / 2.0
geometries = [
    SRVConfig(name="srv_1", length=300 * conversion_factor, height=50 * conversion_factor, center_x=center_x_val - 90 * conversion_factor, center_y=fracture_y1_coords, materials=srv_mats),
    HydraulicFractureConfig(name="hf_1", length=250 * conversion_factor, height=0.2 * conversion_factor, center_x=center_x_val - 90 * conversion_factor, center_y=fracture_y1_coords, materials=fracture_mats),
    SRVConfig(name="srv_2", length=300 * conversion_factor, height=50 * conversion_factor, center_x=center_x_val - 90 * conversion_factor, center_y=fracture_y2_coords, materials=srv_mats),
    HydraulicFractureConfig(name="hf_2", length=250 * conversion_factor, height=0.2 * conversion_factor, center_x=center_x_val - 90 * conversion_factor, center_y=fracture_y2_coords, materials=fracture_mats),
    SRVConfig(name="srv_3", length=300 * conversion_factor, height=50 * conversion_factor,  center_x=center_x_val - 90 * conversion_factor, center_y=fracture_y3_coords, materials=srv_mats),
    HydraulicFractureConfig(name="hf_3", length=250 * conversion_factor, height=0.2 * conversion_factor, center_x=center_x_val - 90 * conversion_factor, center_y=fracture_y3_coords, materials=fracture_mats)
]
...
builder.add_nodeset_by_coord(nodeset_op_name="injection_1", new_boundary_name="injection_1", coordinates=(center_x_val - 90 * conversion_factor, fracture_y1_coords, 0))
builder.add_nodeset_by_coord(nodeset_op_name="injection_2", new_boundary_name="injection_2", coordinates=(center_x_val - 90 * conversion_factor, fracture_y2_coords, 0))
builder.add_nodeset_by_coord(nodeset_op_name="injection_3", new_boundary_name="injection_3", coordinates=(center_x_val - 90 * conversion_factor, fracture_y3_coords, 0))
...
builder.add_postprocessor(PointValueSamplerConfig(name="pp_inj_1", variable="pp", point=(center_x_val - 90 * conversion_factor, fracture_y1_coords, 0)))
builder.add_postprocessor(PointValueSamplerConfig(name="pp_inj_2", variable="pp", point=(center_x_val - 90 * conversion_factor, fracture_y2_coords, 0)))
builder.add_postprocessor(PointValueSamplerConfig(name="pp_inj_3", variable="pp", point=(center_x_val - 90 * conversion_factor, fracture_y3_coords, 0)))
