...
# --- PROJECT SETUP ---
project_name = "112_optimizer_test"
...
def objective(**params):
    ...
    if not (params['fracture_perm'] > params['srv_perm'] > matrix_perm):
        ...
        with open(summary_log_path, 'a', newline='') as f:
            writer = csv.writer(f, delimiter='\t')
            writer.writerow([iteration_counter, params.get('srv_perm'), params.get('fracture_perm'), matrix_perm, params.get('srv_height_ft'), 1e10])
        return 1e10
    ...
    dss_data_path_abs = os.path.join(CWD, "data/fiberis_format/s_well/dss_data/Mariner 14x-36-POW-S - RFS strain change.npz")
    ...
    try:
        ...
        with open(summary_log_path, 'a', newline='') as f:
            writer = csv.writer(f, delimiter='\t')
            writer.writerow([iteration_counter, params.get('srv_perm'), params.get('fracture_perm'), matrix_perm, params.get('srv_height_ft'), misfit_val])
        ...
        try:
            ...
            strain_dataframe_plot_copy.select_depth(center_depth - 10 * conversion_factor, center_depth + 10 * conversion_factor)
            ...
            im2 = strain_dataframe_plot_copy.plot(ax=ax2, use_timestamp=False, cmap='bwr', clim=[-strain_max * 1e6, strain_max * 1e6], method='pcolormesh')
            ...
            example_DSS_center = Data1D(taxis=DSSdata_calibrated.taxis, data=DSS_center_data, name=f"DSS Strain at {DSS_frac_center} ft", start_time=DSSdata_calibrated.start_time)

            DSS_upper_data = DSSdata_calibrated.get_value_by_depth(DSS_frac_center + 3)
            example_DSS_upper = Data1D(taxis=DSSdata_calibrated.taxis, data=DSS_upper_data, name=f"DSS Strain at {DSS_frac_center -3} ft", start_time=DSSdata_calibrated.start_time)

            DSS_upper2_data = DSSdata_calibrated.get_value_by_depth(DSS_frac_center + 6)
            example_DSS_upper2 = Data1D(taxis=DSSdata_calibrated.taxis, data=DSS_upper2_data, name=f"DSS Strain at {DSS_frac_center -6} ft", start_time=DSSdata_calibrated.start_time)
        ...
        with open(summary_log_path, 'a', newline='') as f:
             writer = csv.writer(f, delimiter='\t')
             writer.writerow([iteration_counter, params.get('srv_perm'), params.get('fracture_perm'), matrix_perm, params.get('srv_height_ft'), 'ERROR'])
        return 1e10
