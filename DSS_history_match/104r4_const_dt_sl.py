...
# This script now demonstrates saving a configured ModelBuilder and then loading it to run a simulation.
...
#%% Configure MOOSE model and save the builder instance
...
gauge_data_for_moose = injection_pressure_dataframe.copy()
...
gauge_locs_m = gauge_locs * conversion_factor
for i, gauge_y in enumerate(gauge_locs_m):
    builder.add_postprocessor(PointValueSamplerConfig(name=f"pp_gauge_{i}", variable="pp", point=(center_x_val, gauge_y, 0)))
    builder.add_postprocessor(PointValueSamplerConfig(name=f"strain_gauge_{i}", variable="strain_yy", point=(center_x_val, gauge_y, 0)))

y_min_mesh = min(fracture_y_coords) - 50 * conversion_factor
y_max_mesh = max(fracture_y_coords) + 50 * conversion_factor
...
# --- Save the configured ModelBuilder instance ---
print("\n--- Saving ModelBuilder State ---")
save_dir = "output/1027_sl_modelbuilder"
os.makedirs(save_dir, exist_ok=True)
builder_save_path = os.path.join(save_dir, "dss_const_dt_model.npz")
builder.save_npz(builder_save_path)
print("--- ModelBuilder State Saved ---")


#%% --- Load the builder and Run Simulation ---
print("\n--- Loading ModelBuilder State and Running Simulation ---")

# Load the builder from the saved file
loaded_builder = ModelBuilder.load_npz(builder_save_path)

# Generate the input file from the loaded builder to ensure it's the same
input_file_name = os.path.join(output_dir, "DSS_constDT_from_loaded_builder.i")
loaded_builder.generate_input_file(input_file_name)
print(f"Successfully generated input file from loaded builder: {input_file_name}")
loaded_builder.plot_geometry()


# --- Run Simulation using the input file from the loaded builder ---
