...
def run_full_multi_fracture_simulation():
    ...
    THIS SCRIPT IS CORRECTED by removing the redundant 'output_vector=True'
    argument, which was causing the ModelBuilder to generate an 'unused parameter'
    error in the MOOSE input file.
    """
    # --- 1. Basic Setup ---
    output_dir = "output/moose_full_sim_output_fixed_v13"
    os.makedirs(output_dir, exist_ok=True)
    input_file = os.path.join(output_dir, "full_multi_frac_sim_fixed.i")

    builder = ModelBuilder(project_name="FullMultiFractureSimFixed_v13")
    ...
    dummy_pressure_curve = Data1D(taxis=np.array([0, 3600]), data=np.array([2.7E7, 4.0E7]))
    ...
    # CORRECTED: Removed the redundant 'output_vector=True' argument.
