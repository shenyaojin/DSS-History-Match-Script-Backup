# English comments are used as requested.
import os
import numpy as np
...
from fiberis.moose.config import (
    ...
    # Import the new, elegant config classes for the adaptive timestepper
    AdaptiveTimeStepperConfig, TimeStepperFunctionConfig
)
from fiberis.analyzer.Data1D.core1D import Data1D


def run_full_multi_fracture_simulation():
    """
    Builds and runs a complete multi-fracture simulation with full physics.
    This version demonstrates how to use the newly implemented adaptive
    timestepper functionality to control the timestep during specific intervals.
    """
    # --- 1. Basic Setup ---
    output_dir = "output/moose_full_sim_output_fixed_v15"
    os.makedirs(output_dir, exist_ok=True)
    input_file = os.path.join(output_dir, "full_multi_frac_sim_fixed.i")

    builder = ModelBuilder(project_name="FullMultiFractureSimFixed_v15")
    ...
    # Use np.linspace for a smooth injection pressure ramp
    injection_time_points = np.linspace(0, 3600, 100)
    injection_pressure_points = np.linspace(2.7E7, 4.0E7, 100)
    dummy_pressure_curve = Data1D(taxis=injection_time_points, data=injection_pressure_points)
    ...
    # --- 5. Solver and Output ---
    # NEW: Define the adaptive timestep control functions
    dt_control_func = TimeStepperFunctionConfig(
        name='dt_control_func',
        x_values=[0, 2000, 2500, 3600],
        y_values=[100, 50, 100]  # dt is 100, then 50, then 100 again
    )

    # Package the function into the main adaptive stepper config
    adaptive_config = AdaptiveTimeStepperConfig(functions=[dt_control_func])

    # Pass the config to the executioner. The builder will handle the rest.
    builder.add_executioner_block(
        end_time=3600,
        dt=100,  # In adaptive DT, I set it as initial dt
...
if __name__ == "__main__":
    run_full_multi_fracture_simulation()
