...
#%% Global parameters list, change the value to xxx while upload the script to github
range_min = 15000 # 0
range_max = 16750 # 1750
...
#%% Do process on DAS data.
filtered_DAS_data = DASdata.copy()
filtered_DAS_data.data = np.where(DASdata.data > 0, DASdata.data, 0)
# Get the LF-DAS converted pressure data
DASdata_pressure = filtered_DAS_data.copy()
# Convert the LF-DAS data to pressure
DASdata_pressure.data = DASdata_pressure.data / 10430.4 * 2.799e-8
DASdata_pressure.data = np.cumsum(filtered_DAS_data.data, axis=1)

# fig, ax = plt.subplots(figsize=(4, 6))
# cx = np.array([-1, 1])
# img1 = DASdata_pressure.plot(ax=ax, useTimeStamp=False, cmap='bwr', aspect='auto')
# img1.set_clim(cx * 1e6)
# # Set title
# ax.set_title("LF-DAS data")
# plt.show()
...
for val_iter in gauge_md_data.data[ind]:
    chan_val = DASdata_pressure.get_value_by_depth(val_iter)
...
for gauge_iter in gauge_num:
    ...
    gauge_data_tmp.data = gauge_data_tmp.data / E
    gauge_data_all.append(gauge_data_tmp)
    pressure_val_all.append(gauge_data_tmp.data)

#%% Co plot the DAS and gauge data
# plt.figure()
# plt.plot(gauge_data_all[1].taxis, gauge_data_all[1].data)
# plt.twinx()
# plt.plot(DASdata.taxis, DASchan_all[1], 'r')
# plt.show()

#%% Linear regression
from scipy.stats import linregress
gauge_pressure_flat = np.array(pressure_val_all).flatten()
DAS_pressure_flat = np.array(DASpressure_all).flatten()

slope, intercept, _, _, _ = linregress(DAS_pressure_flat, gauge_pressure_flat)

#%% Convert the DAS data to pressure
DASdata_pressure.data = slope * DASdata_pressure.data + intercept

#%% QC the DAS data
# Perform QC
plt.figure()
plt.plot(DASdata.taxis, DASdata_pressure.data[-1] * slope + intercept, 'r')
# plt.plot(gauge_data_all[1].taxis, gauge_data_all[1].data, 'b')
plt.title("QC: DAS converted pressure vs gauge data")
plt.xlabel("Time")
plt.ylabel("Pressure")
plt.show()
#%% Get some parameters
