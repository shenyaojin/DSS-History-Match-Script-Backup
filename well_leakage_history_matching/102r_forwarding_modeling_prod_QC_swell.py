...
# Set up the mesh
mesh = np.arange(np.min(frac_hit_md_stg7) - 6000, np.max(frac_hit_md_stg7) + 6000, 10)
gauge_idx = 5
...
for diff_drop_iter in diffusivity_array_all:
    ...
    plt.show()
    # Pack the result
    simulator.pack_result(filename=f"output/0218_forward_modeling/{diffusivity_drop_ratio[flag]}_gauge{gauge_idx+1}.npz")
