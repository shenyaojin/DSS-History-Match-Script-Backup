...
# Define the label tmp
label_tmp = ["Field data", "Simulated drawdown\n (min D = 1 × baseline)", "Simulated drawdown\n (min D = 1e-5 × baseline)"]
flag = 0
for idx in [0, 2, 4]:
    plt.plot(axis, pressure_drop_down[idx], label=label_tmp[flag], marker='o', linestyle='-')
...
plt.gca().set_yticks([])
plt.savefig(f"figs/manuscript/production/dropdown_to202051_noscalar.png")
plt.show()